...
class ServerHandler(private val statusQueue: ArrayList<ModelStatus>): HttpHandler {
    override fun handle(exchange: HttpExchange) {
        thread {
            println("I: handle request [${exchange.requestMethod}: ${exchange.requestURI}]")
            try {
                val endpoint = getEndpoint(exchange)
                when (exchange.requestMethod) {
                    "OPTIONS" -> reply("", 200, exchange)
                    "GET" -> {
                        when (endpoint) {
    ...
    private fun reply(response: String, code: Int, exchange: HttpExchange) {
        val json = ServerMapper.getMapper().writeValueAsString(response)
        println("R: reply [$json] with code [$code]")
        exchange.responseHeaders.set("Access-Control-Allow-Origin", "*")
        exchange.responseHeaders.set("Access-Control-Allow-Methods", "GET, POST, DELETE, OPTIONS")
        exchange.responseHeaders.set("Access-Control-Allow-Headers", "Content-Type, Model-Name")
    ...
    private fun servicePostTrain(exchange: HttpExchange) {
        ...
        /** Check model exists **/
        if(ModelSerialization.exists(modelDTO.modelName)) {
            reply("E: Model with name ${modelDTO.modelName} already exists.", 400, exchange)
            return
        }

        /** Check current training models limit **/
        statusQueue.count {
        ...
        try {
            for (epoch in 0 until modelDTO.epochs) {
                ...
                val response = "R: Model ${status.modelName} reached epoch (${epoch + 1}/${modelDTO.epochs}) with accuracy ${
                    "%.3f".format(
                        Locale.ENGLISH,
                        accuracy * 100
                    )
        ...
        /** Evaluate model **/
        updateStatus(status, ModelStatus.Status.TRAINING, "I: Test model.")
        try {
            val matrix = Evaluation.confusionMatrix(y, predictedLabels)
            updateStatus(status, ModelStatus.Status.READY, "R: Training data confusion matrix:\n$matrix")
            ...
            val (testAccuracy, testMatrix) = model.test(xTest, yTest)
            updateStatus(status, ModelStatus.Status.READY, "R: Test data accuracy: $testAccuracy")
