...
fun main(args: Array<String>) {
    val modelName = if(args.isEmpty()) "digits_25e_1c_1d" else args[0]
    val testSizeException = if(args.size > 1) args[1].toInt() else 18800

    val model = ModelSerialization.load(modelName)
    model.build()
    val xTest = DataLoader.loadImageData("${Utils.DATA_PATH}/${Utils.TEST_IMAGE_DIGITS_DATA_FILENAME}", testSizeException)
    val yTest = DataLoader.loadLabelData("${Utils.DATA_PATH}/${Utils.TEST_LABEL_DIGITS_DATA_FILENAME}", testSizeException)
