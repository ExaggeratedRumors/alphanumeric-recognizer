...
fun main() {
    ...
    val xTrain = DataLoader.loadImageData("${Utils.DATA_PATH}/${Utils.TRAIN_IMAGE_DIGITS_DATA_FILENAME}", 1000)
    println("R: Load ${xTrain.data.size} train images.")
    val yTrain = DataLoader.loadLabelData("${Utils.DATA_PATH}/${Utils.TRAIN_LABEL_DIGITS_DATA_FILENAME}", 1000)
    println("R: Load ${yTrain.labels.size} train labels.")
    val xTest = DataLoader.loadImageData("${Utils.DATA_PATH}/${Utils.TEST_IMAGE_DIGITS_DATA_FILENAME}", 1000)
    println("R: Load ${xTest.data.size} test images.")
    val yTest = DataLoader.loadLabelData("${Utils.DATA_PATH}/${Utils.TEST_LABEL_DIGITS_DATA_FILENAME}", 1000)
